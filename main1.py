...
enemies = []
lasers = []
var_threshold_enemy = [350,750]
var_enemy = [[100,450],[500,850]]
# initialize pygame
pygame.init()
...
available_bullets = 6
available_bullets_1 = available_bullets-1
num_bullets_fired =0
num_enemies_killed=0
num_lasers_fired=0
score_sprint = 0
bullets_left = 0
bullets_fired_by_enemies = 0
# game sounds
pause_sound = None
level_up_sound = None
...
def scoreboard():
    ...
    temp1 = available_bullets - 1
    if temp1 <= 0:
        temp1 = 0
    score_sprint = font.render("Enemies Killed : " + str(score), True, (255, 255, 255))
    bullets_left = font.render("Bullets Remaining : "+ str(temp1) + " | " + ("* "*temp1), True, (255, 255, 255))
    bullets_fired_by_enemies = font.render("Casualties : "+ str(num_lasers_fired), True, (255, 255, 255))
    ...
    window.blit(score_sprint, (x_offset, y_offset+20))
    window.blit(bullets_left, (x_offset, y_offset))
    window.blit(bullets_fired_by_enemies, (x_offset, y_offset + 40))
    #window.blit(difficulty_sprint, (x_offset, y_offset + 60))
    #window.blit(life_sprint, (x_offset, y_offset + 60)
...
#     distance = math.sqrt(math.pow((x2_cm - x1_cm), 2) + math.pow((y2_cm - y1_cm), 2))en
...
def kill_enemy(player_obj, bullet_obj, enemy_obj,laser_obj):
    global score
    global kills
    global difficulty
    ...
    laser_obj.shoot_timer = 0
    enemy_obj.random_chance = np.random.randint(-100,0)
...
def gameover_screen():
    ...
    score_sprint = font.render("Enemies Killed : " + str(score), True, (255, 255, 255))
    #accuracy = font.render("Success Rate : " + str(int(score*100/(available_bullets_1-available_bullets)))+str(" % "), True, (255, 255, 255))
    #bullets_left = font.render("Bullets Remaining : "+ str(available_bullets) + " | " + ("* "*available_bullets), True, (255, 255, 255))
    bullets_fired_by_enemies = font.render("Casualties : "+ str(num_lasers_fired), True, (255, 255, 255))
    ...
    #window.blit(accuracy, (x_offset, y_offset + 20))
    window.blit(bullets_fired_by_enemies, (x_offset, y_offset + 20))
    pygame.display.update()
...
def init_game():
    ...
    for lev in range(level):
        ...
        enemy_obj = Enemy(enemy_reloading_img_path,enemy_img_path, enemy_width, enemy_height, enemy_x, enemy_y, enemy_dx, enemy_dy,
...
while running:
    ...
    for i in range(len(enemies)):
        ...
        # enemy movement
        #enemies[i].reloading = False
        if enemies[i].random_chance > var_enemy[i][0] and enemies[i].random_chance < var_enemy[i][1]:
        ...
        if lasers[i].beamed :
            if i==0:
                ...
                enemies[i].random_chance = np.random.randint(-100,-50)
                lasers[i].y += lasers[i].dy
    ...
    for i in range(len(enemies)):
        ...
        if bullet_enemy_collision:
            ...
            #available_bullets -= 1
            kill_enemy(player, bullet, enemies[i],lasers[i])
