...
var_threshold_enemy = [300, 500]
var_enemy = [[100, 300], [400, 500]]
...
tank_moving = pygame.image.load("C:/Users/darshan19/Downloads/attachments/new/tank_moving.png")
tank_reloading = pygame.image.load("C:/Users/darshan19/Downloads/attachments/new/tank_reloading.png")
drone_moving = pygame.image.load("C:/Users/darshan19/Downloads/attachments/new/drone_moving1.png")
drone_reloading = pygame.image.load("C:/Users/darshan19/Downloads/attachments/new/drone_reloading.png")

#pygame.display.set_icon(tank_moving)
...
# create background
background_img = pygame.image.load("C:\\Users\\darshan19\\Desktop\\AoI\\CNN\\bigger_background.png")  # 800 x 600 px image
...
class Enemy:
    ...
    def __init__(self, img_path, img_reloading_path, width, height, x, y, dx, dy, kill_sound_path,lev):
        self.img_path = img_path
        if lev ==0:
            self.img = pygame.image.load(self.img_path)
            ...
            self.img_reloading = pygame.image.load(self.img_reloading_path)
            self.img_reloading = pygame.transform.scale(self.img_reloading, (120, 120))
        else:
            self.img = pygame.image.load(self.img_path)
            self.reloading = False
            self.img_reloading_path = img_reloading_path
    ...
    def draw(self):
        if self.reloading == True:
            window.blit(self.img_reloading, (self.x, self.y))
        else:
            window.blit(self.img, (self.x, self.y))
...
def make_video(screen):
    ...
    while True:
        ...
        file_name = "C:\\Users\\darshan19\\Desktop\\Atari\\Space-Invaders-Pygame-master\\Space Invaders\\Images\\"+"image" + str_num[-4:] + ".jpg"
        pygame.image.save(screen, file_name)
        # print("In generator ", file_name)  # delete, just for demonstration
        # pygame.time.wait(1000)  # delete, just for demonstration
        yield

save_screen = make_video(window)
def pause_game():
    pause_sound.play()
    scoreboard()
    font = pygame.font.SysFont("freesansbold", 64)
...
def init_game():
    ...
    enemy_img_path = "C:/Users/darshan19/Downloads/attachments/new/tank_reloading.png"  # 64 x 64 px image
    enemy_width = 150
    enemy_height = 150
    ...
    enemy_reloading_img_path = "C:/Users/darshan19/Downloads/attachments/new/tank_moving.png"  # 64 x 64 px image

    enemy_2_img_path = "C:/Users/darshan19/Downloads/attachments/new/drone_moving1.png"  # 64 x 64 px image

    # enemy reloading (number of enemy = level number)
    enemy_2_reloading_img_path = "C:/Users/darshan19/Downloads/attachments/new/drone_reloading.png"  # 64 x 64 px image
    ...
    for lev in range(level):
        if lev ==0:
            ...
            enemy_obj = Enemy(enemy_img_path, enemy_reloading_img_path, enemy_width, enemy_height, enemy_x, enemy_y,
                              enemy_dx, enemy_dy, enemy_kill_sound_path,lev)
            ...
            temp_x =  random.randint(0, (WIDTH - enemy_width))
            temp_y = random.randint(((HEIGHT / 10) * 1 - (enemy_height / 2)), ((HEIGHT / 10) * 4 - (enemy_height / 2)))
            if temp_x not in range(enemy_x-10,enemy_x+10):
                enemy_x2 = temp_x
            else:
                enemy_x2 = temp_x + 50
            if temp_y not in range(enemy_y-10,enemy_y+10):
                enemy_y2 = temp_y
            else:
                enemy_y2 = temp_y + 50
            ...
            enemy_obj = Enemy(enemy_2_img_path, enemy_2_reloading_img_path, enemy_width, enemy_height, enemy_x2, enemy_y2,
                              enemy_dx, enemy_dy, enemy_kill_sound_path,lev)
...
while running:
    ...
    for i in range(len(enemies)):
        ...
        # enemy movement
        # enemies[i].reloading = False
        if enemies[i].random_chance > var_enemy[i][0] and enemies[i].random_chance <= var_enemy[i][1]+30:
            enemies[i].reloading = True
        enemies[i].x += enemies[i].dx * float(2 ** (difficulty - 1))

        # laser movement
        if lasers[i].beamed:
            enemies[i].random_chance = np.random.randint(-50, 0)
    ...
    # enemy
    num_bullets_fired = 0.793 * num_enemies_killed
